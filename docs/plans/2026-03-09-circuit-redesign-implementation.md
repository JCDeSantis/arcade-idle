# CIRCUIT.EXE Redesign Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Replace the random-spawn drag mechanic with a charge-graph edge-routing game where a fixed network of nodes accumulates voltage and the player hops along edges to harvest chains before nodes cascade-overload.

**Architecture:** Full rewrite of `js/games/circuit.js` only. Graph is generated fresh each session (14 nodes, nearest-neighbor edges). Nodes charge over time; player mousedown on a node then moves toward connected neighbors to snap them into a chain; mouseup scores. Overloaded nodes cascade charge to neighbors and score a penalty. Two upgrade description strings updated in `upgrades.js`. Everything else (state, launcher, hub, automation) unchanged.
...
- `js/games/circuit.js` — current file being replaced
- `js/upgrades.js` — two description strings to update
- `js/games/paddle.js` — reference for overlay/loop/initGame/update/render pattern

---

## Task 1: Update Upgrade Descriptions

**Files:**
- Modify: `js/upgrades.js`

### Step 1: Update two description strings

Read `js/upgrades.js`. Find the `circuit_node_lifetime` entry (id matches exactly) and update its `desc` field:

**From:** `'Circuit nodes live +15% longer per level'`
**To:** `'Circuit nodes charge 15% slower per level'`

Find the `circuit_powerup_chance` entry and update its `desc` field:

**From:** `'+10% power-up node spawn rate per level'`
**To:** `'Power-up nodes graft into the circuit more often per level'`
...
git commit -m "fix(circuit): update upgrade descriptions for redesigned mechanic"
```

---

## Task 2: Rewrite circuit.js

**Files:**
- Overwrite: `js/games/circuit.js`

### Step 1: Replace the entire file with the following content

Write this as the complete new `js/games/circuit.js`:
...
Open `http://localhost:8080`. Unlock circuit via DevTools (`state.currencies.lifetimeBits = 200001`), then call `unlockStage3()` manually or wait for the tick. Launch CIRCUIT.EXE and verify:
- Canvas shows a network of ~14 nodes connected by neon edges
- Nodes gradually charge (color shifts cyan → amber → magenta)
- Clicking a node and dragging toward a connected neighbor snaps it into the chain (highlighted white, edge brightens)
- Can only snap nodes connected to the last node in the chain
- Releasing mouse scores the chain (popup shows points), nodes reset to 0% charge
- Fully charged nodes overload: "ARC! -30" popup, node removed, neighbors gain charge
- Session ends at 45s with results screen

### Step 3: Commit

```bash
git add js/games/circuit.js
git commit -m "feat(circuit): redesign as charge-graph edge-routing game"
```
...
- [ ] Network of ~14 nodes renders with edges on launch
- [ ] Nodes charge over time, color shifts cyan → amber → magenta
- [ ] Mousedown on a node starts a chain; moving toward a connected neighbor snaps it
- [ ] Moving toward a non-connected node does nothing
- [ ] Chain multiplier applies (3+ nodes = ×1.5, 5+ = ×2, 7+ = ×3)
- [ ] Releasing mouse scores chain, resets node charges to 0
- [ ] Overloaded node triggers "ARC!" penalty popup, cascades charge to neighbors, removes from graph
- [ ] New nodes graft into network every ~10s
- [ ] Power-up nodes graft in, can be captured mid-chain, trigger correct effects
- [ ] `stabilize` reduces all charge by 30%
- [ ] `surge_harvest` scores all nodes at current charge instantly
- [ ] `amplify` doubles chain score for 8s
- [ ] `freeze` and `reinforce` show in HUD strip and affect charge rates
- [ ] `extend` adds 8s to timer
- [ ] `circuit_node_lifetime` upgrade makes nodes charge slower
- [ ] Session submits result, mastery grows, bits/TD awarded
